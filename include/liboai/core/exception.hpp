...
namespace liboai {
    namespace exception {
        ...
        constexpr const char* _etype_strs_[7] = { "E_FAILURETOPARSE:0x00",  "E_BADREQUEST:0x01",
                                                  "E_APIERROR:0x02",        "E_RATELIMIT:0x03",
        ...
        public:
            ...
            OpenAIException(const OpenAIException& rhs) noexcept
                : m_error_type(rhs.m_error_type), m_data(rhs.m_data), m_locale(rhs.m_locale) {
                this->m_fmt_str =
                    (this->m_locale + ": " + this->m_data + " (" +
                     this->GetETypeString(this->m_error_type) + ")");
            }

            OpenAIException(OpenAIException&& rhs) noexcept
                : m_error_type(rhs.m_error_type),
                  m_data(std::move(rhs.m_data)),
                  m_locale(std::move(rhs.m_locale)) {
                this->m_fmt_str =
                    (this->m_locale + ": " + this->m_data + " (" +
                     this->GetETypeString(this->m_error_type) + ")");
            }
            ...
            OpenAIException(
                ...
                : m_error_type(error_type), m_data(data), m_locale(locale) {
                this->m_fmt_str =
                    (this->m_locale + ": " + this->m_data + " (" +
                     this->GetETypeString(this->m_error_type) + ")");
            }

            auto what() const noexcept -> const char* override { return this->m_fmt_str.c_str(); }
