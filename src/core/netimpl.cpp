...
#if defined(LIBOAI_DEBUG)
                _liboai_dbg(
                    "[dbg] [@%s] Set CURLOPT_PROXYUSERNAME and CURLOPT_PROXYPASSWORD for Session "
                    "(0x%p) to %s and %s.\n",
                    __func__,
                    this,
                    proxyAuth_.GetUsername(protocol),
...
#if defined(LIBOAI_DEBUG)
        _liboai_dbg(
            "[dbg] [@%s] No user supplied WriteCallback. Set CURLOPT_WRITEFUNCTION and "
            "CURLOPT_WRITEDATA for Session (0x%p) to 0x%p and 0x%p.\n",
            __func__,
            this,
            liboai::netimpl::components::writeFunction,
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg(
        "[dbg] [@%s] Set CURLOPT_HEADERFUNCTION and CURLOPT_HEADERDATA for Session (0x%p) to 0x%p "
        "and 0x%p.\n",
        __func__,
        this,
        liboai::netimpl::components::writeFunction,
...
liboai::Response liboai::netimpl::Session::BuildResponseObject() {
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg("[dbg] [@%s] Called ParseResponseHeader() for Session (0x%p).\n", __func__, this);
#endif
    ...
    return liboai::Response{
        std::move(this->url_str), std::move(this->content), std::move(this->status_line),
        std::move(this->reason),  this->status_code,        this->elapsed
    };
}
...
#if defined(LIBOAI_DEBUG)
        _liboai_dbg(
            "[dbg] [@%s] Set CURLOPT_NOBODY and CURLOPT_CUSTOMREQUEST for Session (0x%p) to 0L and "
            "\"GET\".\n",
            __func__,
            this
        );
...
#if defined(LIBOAI_DEBUG)
        _liboai_dbg(
            "[dbg] [@%s] Set CURLOPT_NOBODY, CURLOPT_CUSTOMREQUEST and CURLOPT_HTTPGET for Session "
            "(0x%p) to 0L, nullptr and 1L.\n",
            __func__,
            this
        );
#endif
...
#if defined(LIBOAI_DEBUG)
        _liboai_dbg(
            "[dbg] [@%s] Set CURLOPT_NOBODY, CURLOPT_POSTFIELDS and CURLOPT_CUSTOMREQUEST for "
            "Session (0x%p) to 0L, \"\" and \"POST\".\n",
            __func__,
            this
        );
#endif
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg(
        "[dbg] [@%s] Set CURLOPT_HTTPGET, CURLOPT_NOBODY and CURLOPT_CUSTOMREQUEST for Session "
        "(0x%p) to 0L, 0L and \"DELETE\".\n",
        __func__,
        this
    );
#endif
...
void liboai::netimpl::Session::PrepareDownload(std::ofstream& file) {
    ...
    e[2] = curl_easy_setopt(
        this->curl_,
        CURLOPT_WRITEFUNCTION,
        liboai::netimpl::components::writeFileFunction
    );
    ...
    _liboai_dbg(
        ...
        "and CURLOPT_CUSTOMREQUEST for Session (0x%p) to 0L, 1L, "
        "liboai::netimpl::components::writeFileFunction, &file and nullptr.\n",
        __func__,
        this
    );
#endif
...
void liboai::netimpl::Session::ParseResponseHeader(
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg(
        "[dbg] [@%s] Set CURLOPT_POSTFIELDSIZE_LARGE and CURLOPT_POSTFIELDS for Session (0x%p) to "
        "%lld and \"%s\".\n",
        __func__,
        this,
        static_cast<curl_off_t>(body.str().length()),
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg(
        "[dbg] [@%s] Set CURLOPT_POSTFIELDSIZE_LARGE and CURLOPT_COPYPOSTFIELDS for Session (0x%p) "
        "to %lld and \"%s\".\n",
        __func__,
        this,
        static_cast<curl_off_t>(body.str().length()),
...
void liboai::netimpl::Session::SetMultipart(const components::Multipart& multipart) {
    ...
    #if defined(LIBOAI_DEBUG)
    _liboai_dbg(
        "[dbg] [@%s] Set multipart for Session (0x%p) using curl_mime_addpart() and "
        "CURLOPT_MIMEPOST.\n",
        __func__,
        this
    );
...
std::string liboai::netimpl::CurlHolder::urlDecode(const std::string& s) {
    ...
    char* output =
        curl_easy_unescape(this->curl_, s.c_str(), static_cast<int>(s.length()), nullptr);
    if (output) {
        std::string result = output;
        curl_free(output);
        return result;
...
size_t liboai::netimpl::components::writeUserFunction(
    char* ptr,
    size_t size,
    size_t nmemb,
...
size_t liboai::netimpl::components::writeFunction(
    char* ptr,
    size_t size,
    size_t nmemb,
    std::string* data
...
size_t liboai::netimpl::components::writeFileFunction(
    char* ptr,
    size_t size,
    size_t nmemb,
    std::ofstream* file
...
liboai::netimpl::components::Files::const_iterator
liboai::netimpl::components::Files::begin() const {
    return this->files.begin();
}
...
liboai::netimpl::components::Files::const_iterator
liboai::netimpl::components::Files::cbegin() const {
    return this->files.cbegin();
}

liboai::netimpl::components::Files::const_iterator
liboai::netimpl::components::Files::cend() const {
...
liboai::netimpl::components::Parameters::Parameters(
    const std::initializer_list<Parameter>& parameters
) {
    this->Add(parameters);
}

void liboai::netimpl::components::Parameters::Add(
    const std::initializer_list<Parameter>& parameters
) {
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg("[dbg] [@%s] Built parameter string \"%s\".\n", __func__, parameter_string.c_str());
#endif
...
#if defined(LIBOAI_DEBUG)
    _liboai_dbg("[dbg] [@%s] Set timeout to %ld milliseconds\n", __func__, timeout.Milliseconds());
#endif
...
void liboai::netimpl::Session::SetProxyAuthentication(
    const components::ProxyAuthentication& proxy_auth
) {
...
liboai::netimpl::components::Proxies::Proxies(
...
const char*
liboai::netimpl::components::ProxyAuthentication::GetUsername(const std::string& protocol) {
    return proxyAuth_[protocol].username.c_str();
}

const char*
liboai::netimpl::components::ProxyAuthentication::GetPassword(const std::string& protocol) {
