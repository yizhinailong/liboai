...
auto liboai::FineTunes::create_async(
    const std::string& training_file,
    std::optional<std::string> validation_file,
    std::optional<std::string> model_id,
    std::optional<uint8_t> n_epochs,
...
auto liboai::FineTunes::retrieve_async(const std::string& fine_tune_id) const& noexcept(false)
...
auto liboai::FineTunes::cancel_async(const std::string& fine_tune_id) const& noexcept(false)
    -> liboai::FutureResponse {
    return std::async(std::launch::async, &liboai::FineTunes::cancel, this, fine_tune_id);
}

auto liboai::FineTunes::list_events(
...
auto liboai::FineTunes::list_events_async(
    const std::string& fine_tune_id,
    std::optional<std::function<bool(std::string, intptr_t)>> stream
) const& noexcept(false) -> liboai::FutureResponse {
    return std::async(
...
auto liboai::FineTunes::remove(const std::string& model) const& noexcept(false)
    -> liboai::Response {
    Response res;
    res = this->Request(
        Method::HTTP_DELETE,
...
auto liboai::FineTunes::remove_async(const std::string& model) const& noexcept(false)
    -> liboai::FutureResponse {
