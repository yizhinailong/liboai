...
auto liboai::Conversation::ChangeFirstSystemMessage(std::string_view new_data) & noexcept(false)
    -> bool {
...
auto liboai::Conversation::AddUserData(std::string_view data, std::string_view name) & noexcept(
    false
) -> bool {
    // if data provided is non-empty
    if (!data.empty()) {
...
auto liboai::Conversation::AppendStreamData(
    std::string data,
    std::string& delta,
    bool& completed
) & noexcept(false) -> bool {
...
auto liboai::Conversation::SplitStreamedData(std::string data) const noexcept(false)
...
auto liboai::Conversation::RemoveStrings(std::string& s, std::string_view p) const noexcept(false)
    -> void {
...
auto liboai::Conversation::SplitFullStreamedData(std::string data) const noexcept(false)
    -> std::vector<std::string> {
    if (data.empty()) {
        return {};
    }
...
auto liboai::Conversation::ParseStreamData(
    std::string data,
...
auto liboai::ChatCompletion::create(
...
auto liboai::ChatCompletion::create_async(
    const std::string& model,
    Conversation& conversation,
    std::optional<std::string> function_call,
    std::optional<float> temperature,
...
auto liboai::Functions::AddFunctions(
    std::initializer_list<std::string_view> function_names
) & noexcept(false) -> bool {
...
auto liboai::Functions::AddFunctions(std::vector<std::string> function_names) & noexcept(false)
    -> bool {
    if (function_names.size() > 0) {
        for (auto& function_name : function_names) {
            if (this->GetFunctionIndex(function_name) == -1) {
...
auto liboai::Functions::PopFunctions(
    std::initializer_list<std::string_view> function_names
) & noexcept(false) -> bool {
    if (function_names.size() > 0) {
        for (auto& function_name : function_names) {
...
auto liboai::Functions::PopFunctions(std::vector<std::string> function_names) & noexcept(false)
    -> bool {
    if (function_names.size() > 0) {
        for (auto& function_name : function_names) {
            auto index = this->GetFunctionIndex(function_name);
...
auto liboai::Functions::SetDescription(
...
auto liboai::Functions::SetRequired(
    std::string_view target,
...
auto liboai::Functions::AppendRequired(std::string_view target, std::string_view param) & noexcept(
    false
) -> bool {
    index i = this->GetFunctionIndex(target);
...
auto liboai::Functions::AppendRequired(
    std::string_view target,
    std::initializer_list<std::string_view> params
) & noexcept(false) -> bool {
    index i = this->GetFunctionIndex(target);
...
auto liboai::Functions::SetParameter(
    std::string_view target,
...
auto liboai::Functions::SetParameters(
    std::string_view target,
        ...
        if (!this->_functions["functions"][i].contains("parameters") && parameters.size() > 0) {
            ...
            for (auto& parameter : parameters) {
                if (!this->_functions["functions"][i]["parameters"]["properties"].contains(
                    ...
                    if (parameter.enumeration) {
                        this->_functions["functions"][i]["parameters"]["properties"][parameter.name]
                                        ["enum"] = parameter.enumeration.value();
                    }
                }
            }
...
auto liboai::Functions::PopParameters(
    std::string_view target,
...
auto liboai::Functions::AppendParameter(
    std::string_view target,
    FunctionParameter parameter
) & noexcept(false) -> bool {
    index i = this->GetFunctionIndex(target);
...
auto liboai::Functions::AppendParameters(
    std::string_view target,
    std::initializer_list<FunctionParameter> parameters
) & noexcept(false) -> bool {
    index i = this->GetFunctionIndex(target);
...
auto liboai::Functions::GetFunctionIndex(std::string_view function_name) const& noexcept(false)
    -> liboai::Functions::index {
    index i = 0;
